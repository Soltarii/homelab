...
Future revisions to this document should include:
- Remove mention of specific connection paths between nodes as they are subject to change over time, focus on the general purpose a given node serves.
- Structure per-node documentation in a standardized format, something like the >Purpose:, >Benefits:, >Connections:, etc. Any mention of specific per-node services probably becomes in documentation/inventory/.

# Nodes - External-LAN & Internal-LAN
